...
local smoke_choices = {}
smoke_choices["Smoke"] = {function(player,choice)
  local user_id = vRP.getUserId(player)
...
local smell_choices = {}
smell_choices["Snort"] = {function(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
    if vRP.tryGetInventoryItem(user_id,"cocaine",1) then
	  vRP.varyThirst(user_id,(20))
      vRPclient.notify(player,{"~g~ Snorting Cocaine - you feel thirsty."})
      play_smell(player)
	  SetPedIsDrunk(ped, true)
      vRP.closeMenu(player)
    end
...
lsd_choices["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if vRP.tryGetInventoryItem(user_id,"lsd",1) then
	  vRP.varyThirst(user_id,(20))
      vRPclient.notify(player,{"~g~ Dropping Acid - you feel thirsty."})
      play_lsd(player)
	  SetPedIsDrunk(ped, true)
      vRP.closeMenu(player)
    end
...
items["weed"] = {"Weed","A some weed.",function(args) return smoke_choices end,0.001}
items["cocaine"] = {"Cocaine","Some cocaine.",function(args) return smell_choices end,0.001}
items["lsd"] = {"LSD","Some LSD.",function(args) return lsd_choices end,0.001}
